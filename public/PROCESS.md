...
| 日期 | 改动 | 关键文件 | 验证 | 状态 |
| --- | --- | --- | --- | --- |
| 2026-07-11 | UI 救援：新增独立 `hear-ui.css`，不再把声桥新 UI 继续混在旧 `style.css` 里；补齐 app icon 并让网页 favicon/brand 使用同款图标；语音入口改为“开始收音 / 保存语音样本 / 停止录音 / 停止收音”，无 API key 也能保存样本；三端 App 重打包为 `0.1.1 build 2`；修复手机端首页大标题溢出并提升资源版本到 `20260711g`。 | `hear-ui.css`, `index.html`, `challenge.html`, `training.html`, `passport.html`, `reminders.html`, `assets/hearbridge-app-icon.svg`, `app.js`, `sync_to_termux.js`, `apps/build_hearbridge_apps.sh`, `apps/HearBridge-Android/app/build.gradle`, `apps/HearBridge-Android/app/src/main/res/drawable/ic_launcher.xml`, `apps/HearBridge-iOS/Info.plist`, `apps/HearBridge-iOS/AppIcons/`, `apps/HearBridge-macOS/Info.plist`, `apps/HearBridge-macOS/HearBridgeIcon.icns`, `website/api/apps/manifest.json`, `website/api/apps/client-downloads.json`, `website/zhengzhengstudio.cn/assets/icons/hearbridge-icon-512.png`, `website/api/data/changelogs.json`, `website/zhengzhengstudio.cn/api/changelogs.json` | 本地 Chrome 验证：桌面 1365px、手机 390px 首页与沟通区无横向溢出；`hear-ui.css` 已加载；品牌图标和 favicon 指向 app icon；“保存语音样本”在无 API key 模式可点击；`./apps/build_hearbridge_apps.sh` 成功生成 Android APK、macOS DMG、iOS unsigned IPA；线上 Chrome 390px 截图确认标题不再被截断。 | 完成 |
